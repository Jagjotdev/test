...
if(!file_exists("$cdb/$sname/$sname.json")){
$in='{"url":"'.$url.'","peruser":"'.$peruser.'","ref":"'.$ref.'","cname":"'.$cname.'","sname":"'.$sname.'","con1":"'.$cn1.'","con2":"'.$cn2.'","con3":"'.$cn3.'","con4":"'.$cn4.'","task":"'.$t1.'","typ":"'.$typ.'"}';
$user_code=rand(10000,9999999);
mkdir("$cdb/$sname");
mkdir("$cdb/$sname/refer");
mkdir("$cdb/$sname/user");
mkdir("$cdb/$sname/refercount");
file_put_contents("$cdb/$sname/$sname.json",$in);
file_put_contents("$cdb/$sname/clone$sname.json",$in);
$dv=file_get_contents("$cdb/campid.txt");
$dvv="$sname+$dv";
file_put_contents("$cdb/campid.txt",$dvv);}
 
echo"   <center> <b>
<h4 style='font-weight: 600; font-size: 18px; color:blue; padding-top:3px; padding-bottom:3px; width:100%; border-bottom:1px solid red;' >CAMP CREATED SUCCESSFULLY<br></font></h4></b><br></center>
<center><br>
