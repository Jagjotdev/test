...
<input  type='number' class='input' placeholder='Enter Number to Ban' name='ban'>

 <input type='submit' class='submit' name='submit' value='Ban' >
</form><br>
...
if($count=="10"){
    if(file_exists("admin/ban/$ban.txt")){
        echo '<script>alert("Number Already Banned...!")</script>'; 
echo"<script>window.history.back(); </script>";
    }else{

file_put_contents("admin/ban/$ban.txt","ban");
echo '<script>alert("Number Banned Successfully.")</script>';
    }
    
}else{
   echo '<script>alert("Invalid Number Entered...!")</script>'; 
echo"<script>window.history.back(); </script>";
}
...
<b>Banned Numbers List</b><br> <?php
 
        $path    = "admin/ban/";
$files = scandir($path);
$files = array_diff(scandir($path), array('.', '..'));
foreach($files as $file){
    $bnn=str_replace(".txt","",$file);
    $linkk="unban.php?id=$bnn";
...
echo"
         ...
         <table>
            ...
            <tbody>
             <tr> 
                <td data-label='$bnn'><a href='$linkk'><font style='margin-left:25px;' color='red'><b>Unban</b></font></a></td>
